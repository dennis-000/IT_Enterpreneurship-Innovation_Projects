...
import { Eye, Trash2, Mail, Phone, Calendar, User, GraduationCap, Download, X } from 'lucide-react';
...
export default function AdmissionsManager() {
    ...
    const [statusFilter, setStatusFilter] = useState<'all' | 'pending' | 'reviewed' | 'contacted' | 'enrolled'>('all');
    ...
    const updateStatus = async (id: string, newStatus: AdmissionInquiry['status']) => {
        try {
    ...
    const getStatusColor = (status: AdmissionInquiry['status']) => {
        switch (status) {
    ...
    const exportToCSV = () => {
        const headers = ['Parent Name', 'Email', 'Phone', 'Student Name', 'Student Age', 'Grade Level', 'Message', 'Status', 'Date'];
        const csvContent = [
            headers.join(','),
        ...
        a.href = url;
        a.download = `admission-inquiries-${new Date().toISOString().split('T')[0]}.csv`;
        a.click();
    ...
    return (
        <div className="space-y-4 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <div>
                    <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Admission Inquiries</h2>
            ...
            <div className="bg-white p-3 sm:p-4 rounded-xl border border-gray-200">
                <div className="flex flex-wrap gap-2">
                    {[
                        ...
                        { value: 'reviewed', label: 'Reviewed', count: inquiries.filter(i => i.status === 'reviewed').length },
                        { value: 'contacted', label: 'Contacted', count: inquiries.filter(i => i.status === 'contacted').length },
                        { value: 'enrolled', label: 'Enrolled', count: inquiries.filter(i => i.status === 'enrolled').length }
                    ].map(filter => (
                        <button
                            key={filter.value}
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                                    <div className="flex-1">
                                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-3">
                                            <div className="flex items-center space-x-2 sm:space-x-3">
                                                <div className="bg-royal-100 p-2 rounded-lg flex-shrink-0">
                                                    <GraduationCap className="w-4 h-4 sm:w-5 sm:h-5 text-royal-600" />
                                                </div>
                                                <div>
                                                    <h3 className="text-base sm:text-lg font-semibold text-gray-800">{inquiry.parent_name}</h3>
                                                    <p className="text-xs sm:text-sm text-gray-600">{inquiry.student_name} - {inquiry.grade_level}</p>
                                                </div>
                                            </div>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 mb-3">
                                            ...
                                            <div className="flex items-center space-x-2 text-xs sm:text-sm text-gray-600">
                                                ...
                                                <span>Student Age: {inquiry.student_age}</span>
                                            </div>
                                            <div className="flex items-center space-x-2 text-xs sm:text-sm text-gray-600">
                                        ...
                                        <p className="text-gray-700 text-xs sm:text-sm bg-gray-50 p-2 sm:p-3 rounded-lg">
                                            {inquiry.message}
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex flex-row sm:flex-col gap-2 sm:gap-2 sm:ml-4">
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => updateStatus(inquiry.id, e.target.value as AdmissionInquiry['status'])}
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center mb-4 sm:mb-6">
                            <h3 className="text-lg sm:text-xl font-bold text-gray-800">Inquiry Details</h3>
                            <button
                        ...
                        <div className="space-y-4 sm:space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                                <div>
                                    <h4 className="font-semibold text-gray-800 mb-2 text-sm sm:text-base">Parent Information</h4>
                                    <div className="space-y-2 text-xs sm:text-sm">
                                        <p className="break-words"><span className="font-medium">Name:</span> {selectedInquiry.parent_name}</p>
                                ...
                                <div>
                                    <h4 className="font-semibold text-gray-800 mb-2 text-sm sm:text-base">Student Information</h4>
                                    <div className="space-y-2 text-xs sm:text-sm">
                                        <p><span className="font-medium">Name:</span> {selectedInquiry.student_name}</p>
                                        <p><span className="font-medium">Age:</span> {selectedInquiry.student_age}</p>
                                        <p><span className="font-medium">Grade Level:</span> {selectedInquiry.grade_level}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="bg-gray-50 p-3 sm:p-4 rounded-lg">
                                    <p className="text-gray-700 text-xs sm:text-sm">{selectedInquiry.message}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                                ...
                                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                                    <select
                                        ...
                                        onChange={(e) => updateStatus(selectedInquiry.id, e.target.value as AdmissionInquiry['status'])}
